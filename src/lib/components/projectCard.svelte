  ...
  <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="flex items-start gap-4 mb-4">
        <span class="text-neutral-600 font-mono text-sm mt-1">
          {String(index + 1).padStart(2, '0')}
        </span>
        <div class="flex-1 min-w-0">
          <h3 class="text-xl md:text-2xl font-semibold text-white group-hover:text-gradient transition-all duration-300 truncate">
            {project.name}
          </h3>
          {#if plainDescription}
            <p class="text-neutral-500 mt-2 line-clamp-2 text-sm md:text-base">
              {plainDescription}
            </p>
          {/if}
        </div>
      ...
      {#if techStack.length > 0}
        <div class="flex flex-wrap gap-2 ml-8">
          {#each techStack as tech}
            <span class="px-3 py-1 text-xs font-mono bg-neutral-800/50 border border-neutral-700/50 rounded-full text-neutral-400">
              {tech}
            </span>
          {/each}
        </div>
    ...
    <div class="flex items-center gap-3 ml-8 md:ml-0">
      {#if project.github_url}
        <a 
          ...
          class="p-2.5 rounded-lg border border-neutral-800 text-neutral-500 hover:text-white hover:border-neutral-600 hover:bg-neutral-800 transition-all duration-200"
      ...
      <a 
        ...
        class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-neutral-800 border border-neutral-700 rounded-lg hover:bg-neutral-700 hover:border-neutral-600 transition-all duration-200"
      >
        Details
