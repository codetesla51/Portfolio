...
<script>  
 ...
 export let bioText = [
...
</script>  <section id="about" class="relative py-20 overflow-hidden">  
  <!-- Enhanced background -->  
  ...
  <div class="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(var(--acc-rgb),0.03)_0%,transparent_50%)]"></div>    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">  
    <!-- Section Header -->  
    <div class="text-center mb-16" data-aos="fade-up">  
<Heading
  badge="Introduction"
  heading="Engineer in Progress"
  subtext="Sharpening my craft one system at a time."
/>
      </div>  
    ...
    <div class="bg-card/90 backdrop-blur-lg rounded-2xl shadow-2xl overflow-hidden border-2 border-acc/30 hover:border-acc/50 transition-all duration-500">  
      ...
      >  
        ...
        {#if inView && currentLine < bioText.length}  
          <div class="flex">  
            <span class="text-acc/70 mr-4 select-none w-8 text-right text-xs">{(bioTerminalLines.length + 1).toString().padStarta(2, '0')}</span>  
