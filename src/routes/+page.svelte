<script>
  ...
  const articles = [
    {
      title: "Stop Writing Bash Scripts, There's a Better Way",
      url: "https://dev.to/uthman_dev/stop-writing-bash-scripts-theres-a-better-way-50en"
    },
    {
      title: "Building Git from Scratch in Go: What I Learned About Version Control Internals",
      url: "https://dev.to/uthman_dev/building-git-from-scratch-in-go-what-i-learned-about-version-control-internals-4dih"
    },
    {
      title: "How Buffer Pooling Doubled My HTTP Server's Throughput (4000-7721 RPS)",
      url: "https://dev.to/uthman_dev/how-buffer-pooling-doubled-my-http-servers-throughput-4000-7721-rps-3i0g"
    },
    {
      title: "Building an HTTP Server from TCP Sockets: 250-4000 RPS",
      url: "https://dev.to/uthman_dev/building-an-http-server-from-tcp-sockets-250-4000-rps-2m93"
    },
    {
      title: "Building a Terminal Calculator That Actually Does Logic - Axion",
      url: "https://dev.to/uthman_dev/building-a-terminal-calculator-that-actually-does-logic-axion-1p0m"
    }
  ];
...
<svelte:head>
  <title>Uthman Oladele - Go Backend Engineer</title>
  <meta name="description" content="Go backend engineer building APIs, distributed systems, and developer tools with PostgreSQL, Redis, and production-focused architecture.">
  <link rel="canonical" href="https://devuthman.vercel.app/">
  <meta property="og:type" content="website">
  <meta property="og:title" content="Uthman Oladele - Go Backend Engineer">
  <meta property="og:description" content="Go backend engineer building APIs, distributed systems, and developer tools with PostgreSQL, Redis, and production-focused architecture.">
  <meta property="og:url" content="https://devuthman.vercel.app/">
  <meta property="og:image" content="https://avatars.githubusercontent.com/u/149274873?v=4">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="Uthman Oladele - Go Backend Engineer">
  <meta name="twitter:description" content="Go backend engineer building APIs, distributed systems, and developer tools with PostgreSQL, Redis, and production-focused architecture.">
  <meta name="twitter:image" content="https://avatars.githubusercontent.com/u/149274873?v=4">
</svelte:head>
...
<section id="hero" class="relative bg-canvas font-mono overflow-hidden">
  <div class="max-w-5xl mx-auto px-5 sm:px-6 md:px-16 lg:px-24 pt-12 pb-10 md:pt-16 md:pb-14 flex flex-col items-center text-center">
    ...
    <h1 class="mb-10 w-full">
      ...
      <div class="ascii-name flex justify-center" aria-hidden="true">
        <svg viewBox="0 0 620 84" preserveAspectRatio="xMidYMid meet" style="width: min(100%, 620px); height: auto; display: block; max-width: 100%;">
          <text x="310" y="12" text-anchor="middle" style="font-family: 'Geist Mono', monospace; font-size: 11px; white-space: pre; user-select: none;" fill="currentColor">
            <tspan x="310" dy="0">██╗   ██╗████████╗██╗  ██╗███╗   ███╗ █████╗ ███╗   ██╗</tspan>
            <tspan x="310" dy="14">██║   ██║╚══██╔══╝██║  ██║████╗ ████║██╔══██╗████╗  ██║</tspan>
            <tspan x="310" dy="14">██║   ██║   ██║   ███████║██╔████╔██║███████║██╔██╗ ██║</tspan>
            <tspan x="310" dy="14">██║   ██║   ██║   ██╔══██║██║╚██╔╝██║██╔══██║██║╚██╗██║</tspan>
            <tspan x="310" dy="14">╚██████╔╝   ██║   ██║  ██║██║ ╚═╝ ██║██║  ██║██║ ╚████║</tspan>
            <tspan x="310" dy="14"> ╚═════╝    ╚═╝   ╚═╝  ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝</tspan>
    ...
    <div class="mt-8 flex flex-col items-center gap-4">
      <div class="flex items-center gap-3">
        ...
        <a href="https://x.com/uthman_dev" target="_blank" rel="noopener noreferrer" class="icon-link text-mute" aria-label="X">
        ...
        <a href="https://dev.to/uthman_dev" target="_blank" rel="noopener noreferrer" class="icon-link text-mute" aria-label="Dev.to">
          <svg class="w-[18px] h-[18px]" fill="currentColor" viewBox="0 0 24 24"><path d="M7.42 10.05c-.18-.16-.46-.23-.84-.23H6v4.36h.58c.37 0 .65-.08.84-.23.21-.16.31-.42.31-.79v-2.32c0-.37-.1-.63-.31-.79zM0 4.94v14.12h24V4.94H0zM8.56 15.3c-.44.58-1.06.77-1.98.77H4.69V8.53h1.93c.92 0 1.54.19 1.98.77.38.5.56 1.18.56 2.02v1.96c0 .84-.18 1.52-.56 2.02h-.04zm4.01-5.87c0 .59-.47 1.06-1.06 1.06s-1.06-.47-1.06-1.06.47-1.06 1.06-1.06 1.06.47 1.06 1.06zm-.04 6.34h-2.09V10.3h2.09v5.47zm6.44-2.04c0 1.39-.52 2.17-1.93 2.17-1.02 0-1.7-.51-1.93-1.23l1.15-.66c.09.37.36.77.81.77.47 0 .73-.26.73-.77v-4.71h1.17v4.43z"/></svg>
        </a>
        <a href="https://www.linkedin.com/in/oladele-usman-4a56833b1" target="_blank" rel="noopener noreferrer" class="icon-link text-mute" aria-label="LinkedIn">
          ...
          <div class="flex items-start justify-between gap-4 sm:gap-6">
            <div class="flex-1 min-w-0">
              <div class="flex items-baseline gap-2.5 flex-wrap">
                ...
                {/if}
                <span class="hidden sm:inline-flex items-center gap-1 text-[10px] tracking-wide text-ash">
                  <span class="w-1 h-1 rounded-full bg-ash/60"></span>
                  {project.tech_stack[0]}
                </span>
              </div>
              <p class="mt-2.5 text-[13px] sm:text-[14px] leading-[1.6] text-body max-w-2xl">
                {project.description}
              </p>
              <div class="mt-3 flex flex-wrap items-center gap-1.5">
                {#each project.tech_stack.slice(0, 4) as tech}
                  <span class="text-[10px] tracking-[0.04em] text-mute border border-hairline rounded-full px-2 py-0.5 bg-canvas group-hover:bg-surface-soft/50 transition-colors">{tech}</span>
                {/each}
              </div>
            </div>
            {#if project.doc_url || project.github_url}
              <div class="hidden sm:flex flex-col items-end gap-1.5 flex-shrink-0 pt-1">
                {#if project.github_url}
                  <a href={project.github_url} target="_blank" rel="noopener noreferrer"
                    class="inline-flex items-center gap-1 text-xs text-mute hover:text-ink transition-colors">GitHub <span class="text-[10px]">↗</span></a>
                {/if}
                {#if project.doc_url}
                  <a href={project.doc_url} target="_blank" rel="noopener noreferrer"
                    class="inline-flex items-center gap-1 text-xs text-mute hover:text-ink transition-colors">Docs <span class="text-[10px]">↗</span></a>
                {/if}
              </div>
            {/if}
          </div>
          {#if project.doc_url || project.github_url}
            <div class="sm:hidden mt-3 flex items-center gap-3">
              {#if project.github_url}
                <a href={project.github_url} target="_blank" rel="noopener noreferrer"
                  class="text-xs text-mute underline decoration-hairline underline-offset-4">GitHub ↗</a>
              {/if}
              {#if project.doc_url}
                <a href={project.doc_url} target="_blank" rel="noopener noreferrer"
                  class="text-xs text-mute underline decoration-hairline underline-offset-4">Docs ↗</a>
              {/if}
            </div>
          {/if}
        </article>
      ...
      {#if data.posts && data.posts.length > 0}
        <div class="border-t border-hairline">
          {#each data.posts as post}
            <a href="/blog/{post.slug}" class="group flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-2 sm:gap-6 py-6 border-b border-hairline hover:bg-surface-soft/20 transition-colors">
              <div class="flex-1 min-w-0">
                <span class="text-[14px] sm:text-[15px] leading-snug text-ink group-hover:text-ink">{post.title}</span>
                {#if post.description}
                  <p class="mt-1 text-[13px] leading-[1.5] text-mute line-clamp-2">{post.description}</p>
                {/if}
                <div class="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-ash">
                  <time datetime={post.date}>{new Date(post.date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</time>
                  <span>·</span>
                  <span>{post.readingTime} min</span>
                  {#if post.tags}
                    <span>·</span>
                    <span class="text-mute">{post.tags.slice(0, 2).join(' · ')}</span>
                  {/if}
                </div>
              </div>
              <span class="hidden sm:inline-flex items-center gap-1 text-xs text-mute group-hover:text-ink flex-shrink-0">Read <span class="text-[10px]">→</span></span>
            </a>
          {/each}
        </div>
      {:else}
        <p class="py-8 text-[13px] text-mute border-t border-hairline">No posts yet — check back soon.</p>
      {/if}
    {/if}
    ...
    {#if activeTab === 'writing'}
      <div class="border-t border-hairline">
        {#each articles as article}
          <a href={article.url} target="_blank" rel="noopener noreferrer" class="group flex items-baseline justify-between gap-4 py-5 border-b border-hairline hover:bg-surface-soft/20 transition-colors">
            <span class="text-[14px] sm:text-[15px] leading-snug text-body group-hover:text-ink pr-4">{article.title}</span>
            <span class="hidden sm:inline text-xs text-mute group-hover:text-ink flex-shrink-0">dev.to ↗</span>
            <span class="sm:hidden text-ash flex-shrink-0">↗</span>
          </a>
        {/each}
      </div>
      <div class="mt-6 flex justify-center">
        <a href="https://dev.to/uthman_dev" target="_blank" rel="noopener noreferrer" class="text-xs text-mute underline decoration-hairline underline-offset-4 hover:decoration-ink">More on dev.to →</a>
      </div>
    {/if}
  </div>
</section>
